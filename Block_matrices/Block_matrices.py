...
from Croy import Punish_overlap, grad_Punish_overlap, gammas_from_centers_vec, force_PSD_func
from Croy import compute_new_eigs_2
...
def minimize(f, x0, jac=None, method= None, 
              options = None, bounds = None,
              ):
    
    fixed = np.zeros(len(x0),dtype=int)
    newbounds = []
    for i,b in enumerate(bounds):
        if abs(b[1]-b[0]) < 1e-10:
            fixed[i]=1
        else:
            newbounds += [bounds[i]]
    #nonlocal _globcount
    ...
    def callback(xi, juck = None):
        nonlocal _globcount
        _globcount += 1
        if _globcount % 4 == 0:
            print('F,it = ', str(np.round(F(xi),5))+', ' +str(_globcount))
    ...
    if jac is not None:
        def JAC(x):
            ...
            xn[fixed == 1] = x0[fixed == 1]
            xn[fixed == 0] = x[:]
            return jac(xn)[fixed == 0]
    else:
        JAC = None
    print('#Variables optimized for:', len(X0))
    
    if 'optimparallel' in options.keys() and options['optimparallel']:
        from optimparallel import minimize_parallel 
        x = minimize_parallel(F, X0, #method = method, 
                              jac = JAC,
                              bounds = newbounds, 
                              options = options, 
                              callback=callback).x
        ...
        x = _sp_minimize(F, X0, method = method, jac = JAC,
                         bounds = newbounds, options = options, 
                         callback=callback).x
    
    Xout = x0.copy()
    Xout[fixed == 0] = x[:]
    return _min_return(Xout)
...
def Build_BTD_purenp(I, J, V, Slices):
    ...
    #Ia = []; Ib = []; Ic = []
    #Ia.append(np.int64(1))
    #Ib.append(np.int64(1))
    #Ic.append(np.int64(1))
...
class block_td:
    ...
    def Gen_Inv_Diag(self,which_n = None,forget_tilde = False, do_gc = False):
        self.Clean_inverse()
        self.iMl_dict = {}
        self.iMl_it = 0
    ...
    def Invert(self, BW = 'all', print_message = False):
        if print_message:
...
class block_sparse:
    ...
    def Block(self,i,j):
        ...
        if hasattr(self, 'Hermitian'):
            try:
                ...
                try:
                    lind = self.ind_dict[(j,i)]
                    return Transpose(self.vals[lind]).conj()
                except:
                    pass
    ...
    def copy(self):
        ...
        if self.has_been_transposed:
            BS = BS[::-1]
        res =  block_sparse(inds_new, vals_new, BS, E_grid = E_grid_new)
        if isinstance(self. Lorentzian_basis, bool):
            res.Lorentzian_basis=True
            res.ei = self.ei.copy()
            res.gamma = self.gamma.copy()
        
        if hasattr(self, 'is_self_energy'):
            res.is_self_energy = True
    ...
    def initialize_lorentzian_fit(self, sampled_xi, NL,fact = 1.0, add_noise_E = False, random_placement = False,
                                  at_extrema = False, de_tol = 0.2, init_E = None, init_G = None):
        #print(sampled_xi)
        ...
        for i,v in enumerate(self.vals):
            ...
            #print(np.diff(sampled_xi[idx_other]))
            #gi = gammas_from_centers_vec(ei,wi, f1[ik,:,:], xi.real)
           # print(shape,v.shape, sampled_xi.shape)
           # out = Matrix_lorentzian_basis(v,#[:,idx_other, :,:], 
            #                              sampled_xi,#[idx_other],
            #                              Ei, Gi)
            mat[:, :, :, :] = 1.0 #= out #np.average(v,axis = 1)[:, np.newaxis, : , :].real
            vals += [mat]
        
        out = block_sparse(inds, vals, self.Block_shape)
        out.Lorentzian_basis = True
        print(hasattr(self,'Hermitian'))
        #if hasattr(self,)
        out.ei    = Ei
    ...
    def fit_sampling_and_Lorenzian(self, xi, Lrz, 
        ...
        #print(Lrz.ei.shape, Lrz.gamma.shape)
        if fit_real_part:
        ...
        # start _1_ 
        # In the following section we assume the matrix being fitted is a dense rectangular block somewhere in the larger matrix
        # the follwing is only consequencial if "force_PSD" is affirmative. Note this probably wont make sense if you fit self-energies
        idx    = np.where(Lrz.is_zero)
        mini   = idx[0].min()
        maxi   = idx[0].max()
        minj   = idx[1].min()
        maxj   = idx[1].max()
        Acc_i  = np.zeros((maxi - mini + 1), dtype = int)
        Acc_j  = np.zeros((maxj - minj + 1), dtype = int)
        it     = 1
        
        # comment after setting up the code comment 
        # The next couple of lines are pretty messy, but are there to 
        # be able to reconstruct the Gamma matrix in the enforcement 
        # of the PSD part of the code. It seems to work, but it is not
        # very readable.
        #
        for i in range(mini, maxi):
            Acc_i[it] += Acc_i[mini:i].sum() + Lrz.Block(i, minj).shape[-2]
        i  = 0
        it = 1
        for j in range(minj, maxj):
            Acc_j[it] += Acc_j[minj:j].sum() + Lrz.Block(mini,j).shape[-1]
        j  = 0
        del i,j, it
        #print('Accumulated indices:')
        #print(Acc_i, Acc_j)
        # stop _1_
        ...
        for i,t in enumerate(t1):
            ...
            _nidxi += [Acc_i[Bi-mini] + bi]
            _nidxj += [Acc_j[Bj-minj] + bj]
        _n_maxN  = max(max(_nidxi),max(_nidxj))+1
        print(_n_maxN)
        ...
        if which_k == None:
            which_k = [i for i in range(nk)]
        if isinstance(force_PSD_tol, (float,int)):
            force_PSD_tol = (force_PSD_tol, )*nk
        ...
        for ik in range(nk):
            if fit_real_part:
                ...
                def f(x):
                    ei = x[0:nl]
                    wi = x[nl:2*nl]
                    gi = gammas_from_centers_vec(ei,wi, f1[ik,:,:], xi.real, exc_idx = exc_idx)
                    if force_PSD:
                        gi = force_PSD_func(gi, _nidxi, _nidxj, _n_maxN, min_tol = force_PSD_tol[ik]#, ei = ei, wi = wi
                                            )
                    ...
                    PARS_v[2,:,:] = gi[:,:]
                    return f_sq + \
                                + delta_error_many_linear_lorentzian(xi.real, f1[ik,:,:].real, PARS_v.real, exc_idx = exc_idx) \
                                + alpha_PO*Punish_overlap(ei,wi)
                def jac(x):
                    ei = x[0:nl]
                    wi = x[nl:2*nl]
                    gi = gammas_from_centers_vec(ei,wi, f1[ik,:,:], xi.real, exc_idx = exc_idx)
                ...
                def f(x):
                    ei = x[0:nl]
                    wi = np.abs(x[nl:2*nl]) # Gamma always positive
                    ...
                    PARS_v[2,:,:] = gi 
                    t4  = time()
                    _v1 = delta_error_many_linear_lorentzian_complex(xi.real, 
                                                                     f1[ik,:,:], 
                                                                     PARS_v, 
                                                                     exc_idx = exc_idx
                                                                     ) 
                    t5  = time()
                    _v2 = alpha_PO*Punish_overlap(ei,wi)
                    t6  = time()
                    if timings:
                        print('Times: ', (t2-t1), (t3-t2), (t4-t3), (t5-t4), (t6-t5))
                    return f_sq_r + f_sq_i + _v1 + _v2
            ...
            if specific_bounds is not None:
            ...
            if not fit_real_part:
                ...
                gi = gammas_from_centers_vec(ei,wi, f1[ik,:,:].real, xi.real, exc_idx = exc_idx)
                if force_PSD:
                    gi = force_PSD_func(gi, _nidxi, _nidxj, _n_maxN, min_tol = force_PSD_tol[ik], ei = ei, wi = wi)
            out.append((ei,wi,gi))
    ...
    def is_positive_semidefinite(self, Egrid, slices, hermitian = True):
        assert hasattr(self, 'Lorentzian_basis')
        M = self.evaluate_Lorentzian_basis(Egrid)
        e, v = M.eig(slices, hermitian = hermitian)
        return e.Get_diagonal(slices)
    
    def iterative_PSD(self,ik = 0, n = 5, nW = 5, eps = 0.0, fact = 0.5, maxit = 50, lbtol = 1e-3, add_last = False):
        assert hasattr(self, 'ei')
        assert hasattr(self, 'gamma')
        
        notot  = 0
        nb     = self.is_zero.shape[0]
        offset = [0]
        for i in range(nb):
            if self.Block(i,i) is not None:
                notot += self.Block(i,i).shape[-1]
                offset+=[self.Block(i,i).shape[-1] + sum(offset[:len(offset)])]
        offset = offset[:-1]
        D    = np.zeros(self.vals[0].shape[:-2] + (notot, notot),dtype=self.dtype)
        idx  = np.where(np.diag(self.is_zero)>0)[0]
        idx0   = idx - idx.min() 
        IJ2sl  = []
        inds   = np.array(self.inds)
        for i,I in enumerate(idx):
            for j,J in enumerate(idx):
                Bij    = self.Block(I,J)
                diff   = np.abs(inds - np.array((I,J))).sum(axis=1)
                if (diff>0).all():
                    continue
                s1, s2 = Bij.shape[-2:]
                sl1    = slice(offset[i], offset[i] + s1)
                sl2    = slice(offset[j], offset[j] + s2)
                IJ2sl += [(np.array((I,J)), (sl1,sl2))]
                D[..., sl1, sl2] = Bij
                if hasattr(self,'Hermitian') and I!=J:
                    D[..., sl2,sl1] = np.conj(Transpose(Bij))
        for ik in range(D.shape[0]):
            M = D[ik].copy()
            #print(M.shape)
            M = compute_new_eigs_2(M, self.ei[ik], self.gamma[ik], n=n, nW=nW, eps=eps, fact=fact, maxit=maxit, lbtol=lbtol, 
                                   add_last = add_last)
            D[ik,:,:,:] = M[:,:,:]
        for i in range(len(IJ2sl)):
            vals = IJ2sl[i]
            idx = np.where(np.abs(np.array(self.inds) - vals[0]).sum(axis=1)<1e-10)[0][0]
            self.vals[idx][:,:,:,:] = D[..., vals[1][0], vals[1][1]]
...
def abssorted_eig_4inds(M, hermitian = False, fixphase = None):
    if hermitian == True:
    ...
    for I in range(i1):
        for II in range(i2):
            ...
            e[I,II,:]     = ei[idx]
            vcop          = vi[:,idx].copy()
            if fixphase is not None:
                fixphase(vcop)
            v[I,II,:,:]   = vcop[:,:]#vi[:,idx]
    
    return e.copy(),v.copy()

def lsa_sort(e,v,es,vs):
    n     = e.shape[0]
    es[0] = e[0]
    vs[0,:,:] = v[0,:,:]
    for i in range(1,n):
        e2,v2 = e[i],v[i]
        Q     = np.abs(vs[i-1].conj().T @ v2)
        assignment = linear_sum_assignment(-Q)[1]
        es[i] = e2[assignment]
        vs[i] = v2[:,assignment]

def lsa_eig_4inds(M, hermitian=False, fixphase = None):
    print('\n Using experimental feature!!! \n')
    print('\n You\'re responsible for the sorting of the Lorentzian positions\n')
    if hermitian == True:
        e,v = np.linalg.eigh(M)
    else:
        e,v  = np.linalg.eig(M)
...
def multisort_eigval(M, hermitian=False, sorting = 'abs', fixphase = None):
    if sorting   == 'abs':
        return abssorted_eig_4inds(M, hermitian=hermitian, fixphase=fixphase)
    elif sorting == 'lsa':
        return lsa_eig_4inds(M, hermitian=hermitian, fixphase=fixphase)

def Inverse_Block_ij(BTD, i, j):
    B = BTD.iM(j,j)
    if i == j:
        return B
    if i>j:
        count = 0
        for k in range(j+1, BTD.N):
            if count == i-j:
                break
            else:
                B = MM(-BTD.Xt(k-1), B)
...
def TriDi_Spectral_function(BTD, Gam):
    nz = np.where(Gam.is_zero>0)
    nnz=len(nz[0])
    vals = []
    inds = []
    for i in range(BTD.N):
        if i == BTD.N-1:
            Ai = np.zeros(BTD.Block(i,i).shape, dtype = np.complex128)
            for count in range(nnz):
                I = nz[0][count]
                J = nz[1][count]
                Ai += MM(MM(Inverse_Block_ij(BTD, i, I), 
                            Gam.Block(I,J)), 
                            Transpose(Inverse_Block_ij(BTD, i, J)).conj()
                          )
            vals += [Ai]
            inds += [[i,i]]
        else:
            Ai = np.zeros(BTD.Block(i  ,i).shape, dtype = np.complex128)
            Bi = np.zeros(BTD.Block(i+1,i).shape, dtype = np.complex128)
            Ci = np.zeros(BTD.Block(i,i+1).shape, dtype = np.complex128)
            for count in range(nnz):
                I = nz[0][count]
                J = nz[1][count]
                Ai += MM(MM(Inverse_Block_ij(BTD, i, I), 
                ...
                Bi += MM(MM(Inverse_Block_ij(BTD, i+1, I), 
                            Gam.Block(I,J)), 
                            Transpose(Inverse_Block_ij(BTD, i, J)).conj()
                         )
                Ci += MM(MM(Inverse_Block_ij(BTD, i, I), 
                            Gam.Block(I,J)), 
                            Transpose(Inverse_Block_ij(BTD, i+1, J)).conj()
                         )
            vals += [Ai, Bi, Ci]
            inds += [[i,i], [i+1,i], [i,i+1]]
    
    return block_sparse(inds, vals, BTD.Block_shape, E_grid = BTD.E_grid)


def Orbital_current(H, S, E, A, aidx, slices):
    from scipy import sparse as sp
    Jij = sp.csr_matrix(H.shape, dtype ='complex')
    k,e = aidx
    #i[(Hβα − ESβα )Aαβ (E) − (Hαβ − ESαβ )Aβα (E)],
    #habssum = abs(H).sum()
    #counter = 0.0
    for i in range(A.Block_shape[0]):
        sd_i,sd_j = slices[i][i]
        Ad = A.Block(i,i)[k,e]
        Hd =     H[sd_i, sd_j]
        Sd = E * S[sd_i, sd_j]
        
        Jij[sd_i, sd_j] = 1j * (( Hd.T - E*Sd.T).toarray() * Ad - ( Hd - E*Sd).toarray() * Ad.T)  
    #    counter+=abs(Hd).sum()
        
        if i < A.Block_shape[0]-1:
            sp_i,sp_j = slices[i][i+1]
            sm_i,sm_j = slices[i+1][i]
            
            Ap = A.Block(i,i+1)[k,e]
            Hp =     H[sp_i, sp_j]
            Sp = E * S[sp_i, sp_j]
            
            Am = A.Block(i+1,i)[k,e]
            Hm =     H[sm_i, sm_j]
            Sm = E * S[sm_i, sm_j]
    #        counter+=abs(Hp).sum()
    #        counter+=abs(Hm).sum()
            
            Jij[sp_i, sp_j] = 1j * (( Hm.T - Sm.T).toarray() * Ap - ( Hp - Sp).toarray() * Am.T)
            Jij[sm_i, sm_j] = 1j * (( Hp.T - Sp.T).toarray() * Am - ( Hm - Sm).toarray() * Ap.T)
    Jij.eliminate_zeros()
    #print(habssum,counter )
    return Jij
